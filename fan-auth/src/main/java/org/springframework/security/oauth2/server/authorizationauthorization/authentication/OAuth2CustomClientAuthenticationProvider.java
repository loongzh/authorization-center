...
import io.githubs.loongzh.auth.constant.Oauth2Constants.TOKEN_SETTINGS;
...
/**
 * OAuth2 Token端点 - 客户端认证器<br/>
 * 注：
 * 扩展支持refresh_token可无需提供client_secret认证（适用于PKCE模式，客户端本地无法安全存储client_secret），
 * 且需要设置RegisteredClient.tokenSettings[allow_public_client_refresh_token]为true（自定义配置）
 *
 * @author luohq
 * @date 2022-03-10
 * @see //org.springframework.security.oauth2.server.authorization.authentication.OAuth2ClientAuthenticationProvider
 */
public final class OAuth2CustomClientAuthenticationProvider implements AuthenticationProvider {
	 ...
	 */
	public OAuth2CustomClientAuthenticationProvider(RegisteredClientRepository registeredClientRepository,
	...
	private Authentication authenticateClientCredentials(Authentication authentication) throws AuthenticationException {
		...
		/** =========== 附加逻辑 - 开始 =========== **/
		//支持refresh_token无client_secret认证，
		//grant_type == refresh_token 且 tokenSetting.allow_public_client_refresh_token，则直接验证通过 */
		String grantType = (String) clientAuthentication.getAdditionalParameters().get(OAuth2ParameterNames.GRANT_TYPE);
		Boolean allowPublicClientRefreshToken = registeredClient.getTokenSettings().getSetting(TOKEN_SETTINGS.ALLOW_PUBLIC_CLIENT_REFRESH_TOKEN);
		if (AuthorizationGrantType.REFRESH_TOKEN.getValue().equals(grantType) && Boolean.TRUE.equals(allowPublicClientRefreshToken)) {
			credentialsAuthenticated = true;
		}
		/** =========== 附加逻辑 - 结束 =========== **/
